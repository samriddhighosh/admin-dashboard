...
const TripDetail = ({loaderData}: Route.ComponentProps) => {
    ...
    const 
    {name, duration, itinerary, travelStyle, groupType, budget, 
        interests, estimatedPrice, description, bestTimeToVisit, weatherInfo, country} 
        = tripData || {};

  return (
    <main className='travel-detail wrapper'>
        <Header title="Trip Details" description='View and edit Ai-egnerated travle plans'/>
        ...
        <section className='container wrapper-md'>
            <header>
                ...
                image="/assest/icons/calendar.svg"
                />
                <InfoPill
                text={itinerary?.slice(0,2).map((item)=> item.location).join(', ')|| ''}
                image="/assest/icons/location-mark.svg"
                />
            </div>
            </header>
